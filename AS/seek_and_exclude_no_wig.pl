...
die "perl $0 <gtf> <junction.bed> <known.out> <novel.out> <filter> <sensitivity>\n" unless(@ARGV eq 6);
...
open KNOWN, "> $ARGV[2]" || die $!;
print KNOWN "Gene_ID\tGene_symbol\tTranscript_ID\tJunction_ID\tJunction_position\tStrand\tLength\tCount\tDensity\tType\n";
open NOVEL, "> $ARGV[3]" || die $!;
print NOVEL "Gene_ID\tGene_symbol\tTranscript_ID\tJunction_ID\tJunction_position\tStrand\tLength\tCount\tDensity\tType\n";
open BED, "$ARGV[1]" || die $!;
<BED>;
while(<BED>){
	chomp;
	my @line = split /\t/;
	next if($line[4] < $ARGV[4]); ## filter reads count less than ARGV[4]
	...
	foreach my $id (sort keys %{$hash{$chr}}){
		...
		for(my $i = 0; $i < @pos; $i += 2){
			if(abs($as_start - $pos[$i]) == $ARGV[5] && abs($as_end - $pos[$i+1]) == $ARGV[5]){
				$flag++;
				my $txt = join "\t", $hash{$chr}{$id}{gene_id}, $hash{$chr}{$id}{gene_name}, $id, $jun_id, "$chr:$block_start,$as_start-$as_end,$block_end", $sign, $as_len, $reads, $as_den, "Known";
				print KNOWN "$txt\n";
				last;
			}
		}
